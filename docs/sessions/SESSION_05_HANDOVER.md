...
1. **[#119](https://github.com/earlyprototype/lucier-gpt2-activ-tensor-reson-experiments/issues/119), which parts of the model do the turning.** Run 18 was read as establishing that the band's lower edge is governed by how far the state's direction swings on each pass. That reading was withdrawn on 2026-08-04 as methodologically unsound (FINDINGS caveat 19), so which quantity governs either edge is now open. What survives is the descriptive profile: the swing rises through the strata below the band and peaks inside it. That swing is the sum of writes from 144 attention heads and 12 feed-forward blocks, collapsed into one arrow by the current measurement, so it cannot tell a concentrated cause from a diffuse one. The issue splits the arrow into per-component shares. The residual stream is additive, so the split is exact rather than heuristic; the limit is that it captures direct contributions only. Cheaper of the two, and it addresses the mechanism question the sweep opened.
...
**The sweep in angular coordinates. WITHDRAWN 2026-08-04 in the part that discriminated between the edges.** This came from a TC objection in session: the single-pass gain is a ratio of magnitudes and says nothing about direction, while everything the project reports at the end is a direction. The engine had been archiving the directional measure all along, as each trial's per-iteration `cos_sim_mean_lag1`. It was converted to degrees turned per pass and re-analysed archive-only (`06_angular_profile.py`, no re-runs). This session then concluded, from a comparison of coefficients of variation across coordinates, that the two edges are governed by different quantities, the lower by the directional turn (CV 0.082 against 0.123 in magnitude) and the upper by the magnitude gain (0.091 against 0.510 in degrees). Both halves of that conclusion are withdrawn. The arithmetic reproduces, but the method does not support it: gain has a floor at 1 rather than 0, so a CV taken about zero is deflated by an offset carrying no information; CV is not invariant under reparameterisation, and in raw cosine instead of degrees the upper edge's verdict reverses; each coordinate reads tight exactly where its profile is flat; the lower-edge number flips its ordering across equally defensible averaging windows (passes 1-1, 1-10, 1-50, 1-100); and the multiplier column takes only two distinct values at the lower edge, being a grid constant. Which quantity governs either edge is OPEN. FINDINGS caveat 19 carries the full withdrawal, issue #17 the successor design. What survives is the raw profile, which is a description rather than a discrimination: in magnitude coordinates the sweep is one smooth decline from 54x to 1.01x and both edges look like the same event; in degrees it is a hill, 45 degrees a pass at 8x, 66 at 24x, 73 at 48x, peaking at 77 inside the band, then collapsing to 9 at 256x. **Below the band the loop is not failing to move; it is moving as much as it ever does.** The mechanism first proposed for the lower edge, that too little changes per pass, does not sit easily with that profile, but nothing here establishes what does govern the edge.
