...
| **H4** per-head resonance ≈ W_OV power iteration | "untested" | **Record is wrong, verified.** Executed 2026-07-25 (PR #29 regeneration), NOT SUPPORTED: 5/144 heads > 0.9. Artifacts committed (`experiments/_DATA/EXP_009/009c_*.pt`). Five places still say untested (FINDINGS ×2, JOURNEY_MAP, TECHNICAL.md:120, README notebook table) plus the notebook's own banner. Needs TC's two rulings in #54. **Resolved 2026-07-31: TC ruled in #54; both rulings landed, and the registered target itself was found to be the wrong spectral object. The corrected-target rescore (run 16) is now the primary record; all six stale statements updated together.** | §5 T0-2, done |
...
> **Reframed 2026-07-31 (TC ruling, #54; run 16).** "Tried and failed" understates what happened. The H4 protocol named the wrong spectral object: the isolated loop iterates W_OV transposed with a direction-preserving rescale, so it lands on that operator's dominant *eigenvector* (a dominant left eigenvector of W_OV), and rescored against that target every settling head lands where the weights predict; the quoted nonlinear-dominance explanation is superseded. The registered proposition was naive rather than falsified-by-nonlinearity. This *strengthens* rather than weakens the conclusion above: even the genuinely linear per-head case was mis-predicted by naming the wrong object, and the full-stack loop, which really is nonlinear (LayerNorm, MLPs, attention all active), gives a weights-only spectrum no license at all. M3, the Jacobian of the actual map, remains the sound replacement.
...
- **Part C's failure mode 3 is neutralised at this specific fixed point.** Within the row-uniform invariant subspace of a position-collapsed state, perturbations leave all queries and keys equal, the softmax stays exactly uniform, and the QK pattern-derivative vanishes: the attention Jacobian restricted to that subspace is exactly the OV circuit under a uniform pattern. A genuine lever the issue doesn't exploit; it also shrinks the *in-manifold* computation to the 768-dim row-uniform subspace (Lanczos over tens to hundreds of jvps, not 768 full passes).
- **It comes with built-in validation the weight-sum version lacks:** |λ₂| must predict the measured settling half-life (5.6 to 11.4 iterations for GPT-2 Small ⇒ |λ₂| ≈ 0.88 to 0.94), and the count of |λ| > 1 directions must be 0 for a genuine attractor. **Scope that count honestly:** the cheap 768-dim spectrum is the *in-manifold* block only; the row-uniform subspace being exactly invariant (#91) says nothing about its *stability*, so the escape-direction count is complete only with the transverse block (perturbations that break row-uniformity, a [T×768] computation), whose contraction rate is precisely M2's position-collapse rate, tying the two metrics together. Report the in-manifold spectrum as in-manifold unless the transverse block is computed. The unexplained ~30-iteration settling latency (M2) is a non-normality/pseudospectrum question the same computation can address.
...
2. **H4 (#54): TC makes the two rulings** (does the regeneration run dispose of the hypothesis; is the `.detach()` deviation inert). Then all six stale statements update together: FINDINGS H4 row and line 883, JOURNEY_MAP §3, TECHNICAL.md:120, the README notebook table, and the notebook's cell-0 banner (README's own Knowledge Graph paragraph already states the truth, contradicting its own table). Closes #54, #25. **Done 2026-07-31: TC ruled (in session, recorded in #54), and the ruling went further than the two questions: the registered comparison target was found to be the wrong spectral object (the loop is power iteration on the transposed write matrix, whose target is that operator's dominant eigenvector, not the top singular vector), so the corrected-target rescore (run 16, `experiments/gpt2_small/12_eigen_rescore.py`) is now the primary record with the original scoring footnoted. Both deviations verified inert inside run 16. All six statements updated.**
...
| 25 | EXP_009c / H4 spectral | Resolves with #54 rulings (T0-2); residue is only the eigenvector-vs-singular-vector addendum. **Done 2026-07-31: the addendum became the primary record (run 16), by TC's ruling.** |
...
| 54 | H4 record contradiction | **T0-2. TC's two rulings, then close.** **Done 2026-07-31: ruled and executed (run 16); close on PR #107 merge.** |
