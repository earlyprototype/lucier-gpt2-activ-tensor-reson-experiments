...
energy-renormalised, iterated to exhaustion) against the published record. It consolidates four parallel survey threads run on 2026-07-21
over the live web, where the network policy allowed: thread A (dynamical systems and iterated transformers), thread B (GPT-2 mechanistic
interpretability), thread C (glitch and undertrained tokens), thread D (text-level loops and the art lineage). Direct fetches failed for
arxiv.org, lesswrong.com, aclanthology.org, and transformer-circuits.pub; github-hosted content was fetchable. Many entries therefore rest
on search snippets covering title, authors, venue, and abstract-level claims, and each reference keeps the verification marker its thread
assigned: [verified: fetched], [verified: search snippet], or [model knowledge: unverified]. Where a URL is not inline it appears in the
Sources section. Where two or more threads converged on the same work independently, that is stated. Terminology follows BELL_PRIMER.md: the
...
2025).** [verified: search snippet] https://aclanthology.org/2025.acl-long.624/ and https://arxiv.org/abs/2502.15208
Three of the four threads located this paper independently, which makes it the established nearest neighbour. Iterated paraphrasing through
...
and the orbits are read as functional. Hao et al., Coconut (arXiv 2412.06769), found by threads A and D independently, feeds the last hidden
...
input-independent fixed point, and engineers against it (STARS, arXiv 2605.26733; Solve the Loop, arXiv 2605.12466). All
[verified: search snippet]. What this literature suppresses or exploits, ATR studies descriptively in a model never trained for the loop.

**Blayney et al., A Mechanistic Analysis of Looped Reasoning Language Models (arXiv 2604.11791).** [verified: search snippet]
...
does. All [verified: search snippet].

**Marcus, Westervelt, Dynamics of Iterated-Map Neural Networks (Phys Rev A 1989).**
[verified: search snippet for the citation; detailed content model knowledge: unverified]
...
**Sussillo, Barak, Opening the Black Box (2013).** [verified: search snippet] https://direct.mit.edu/neco/article/25/3/626/7854
...
**Dong, Cordonnier, Loukas, Attention is Not All You Need (ICML 2021).** [verified: search snippet] https://arxiv.org/abs/2103.03404
Three threads converged on this paper. Pure self-attention converges doubly exponentially to a rank-1, token-uniform state; skip connections
...
configuration set by the input [verified: search snippet]. The oversmoothing line anticipates finding 2 and supports finding 1; none of it
...
independently arrives at ATR's two mechanics (norm-constrained states, repeated block) as design principles. All [verified: search snippet].
...
output-to-input loop, or map attractors. Heimersheim and Turner (LessWrong 2023), found by threads A and B, measured roughly 4.5 percent
...
properties of the renormalised map. Both [verified: search snippet].

## GPT-2 internals

**Elhage et al., A Mathematical Framework for Transformer Circuits (Anthropic 2021).** [verified: search snippet]
https://transformer-circuits.pub/2021/framework/index.html
Found by threads A and B independently. The residual stream is a shared communication channel; heads decompose into QK circuits (where to
...
tuned lens (Belrose et al. 2023, arXiv 2303.08112) documents the logit lens's brittleness and per-model variability. Threads A and B both
located this pair. Finding 6 is a concrete instance of the known blind spot: any lens defines a verbalizable subspace and misses its
complement, where the closed-loop motion lives, so lens-based accounts of the loop would wrongly report stasis. Both
[verified: search snippet].
...
the attended token's logit; GPT-2 Small induction heads 5.5, 5.8, 5.9, 6.9 [verified: fetched, ACDC file], paper [verified: search snippet].
The IOI circuit (indirect object identification; Wang et al. 2022, arXiv 2211.00593 [verified: search snippet]): 26 heads including negative
name movers 10.7 and 11.10, which write against the correct name; head list [verified: fetched]
...
**Kissane, Krzyzanowski, Bloom, Conmy, Nanda 2024, Attention Output SAEs (arXiv 2406.17759).**
[verified: search snippet; repo README fetched] https://arxiv.org/abs/2406.17759, per-head cards [verified: fetched raw HTML]
...
(ACL 2024, arXiv 2402.09221) found dark low-band residual signals that barely affect logits yet carry essential function. All
[verified: search snippet]. These are the strongest precedent for finding 6: GPT-2 components
...
with 73 percent of d in W_U's weakest directions. The Anthropic workspace paper (Transformer Circuits, July 2026) [verified: search snippet]
...
**Anisotropy and outlier dimensions.** Ethayarajh 2019 (arXiv 1909.00512), found by threads A and B: GPT-2 representations occupy a narrow
...
parallels findings 5 and 6. All [verified: search snippet].
...
**Rumbelow and Watkins, SolidGoldMagikarp I to III (LessWrong / Alignment Forum, Feb 2023).** [verified: search snippet]
https://www.lesswrong.com/posts/aPeJE8bSo6rAFoLqg/solidgoldmagikarp-plus-prompt-generation
Three threads converged on this line. The discovery posts: tokens closest to the embedding centroid behave anomalously, cannot be repeated
...
**Land and Bartolo, Fishing for Magikarp (EMNLP 2024, arXiv 2405.05417).** [verified: search snippet + fetched repo]
https://arxiv.org/abs/2405.05417 and https://github.com/cohere-ai/magikarp
Found by threads B and C independently. Systematic undertrained-token detection with about 90 model reports including all four GPT-2 sizes:
1,236 to 2,301 candidates and 36 to 68 verified undertrained tokens per size (excluding special and single-byte)
[verified: fetched, README and results/summary.md]. The repo states that tied-embedding models such as GPT-2 need hand-selected unused-token
...
token (un)embeddings after removing a shared component, not raw norm [model knowledge: unverified detail]. This is the closest
...
(ICLR 2018, arXiv 1702.01417), found by threads B and C, established a dominant frequency-linked, mean-anchored direction in embedding
...
direction makes them the argmax). All [verified: search snippet]. aizi's random-direction baseline
[verified: search snippet, existence only] supplies the caveat that extreme logit lists surface glitch tokens spuriously.
...
2408.04905) [verified: search snippet] reads internal activations, but only as a classifier signal evoked by a glitch token present in the
...
identify tail truncation as the mechanism. All [verified: search snippet]. This is a different object: the learning map iterated across
...
text-to-text map finds fixed points. Perez et al., When LLMs Play the Telephone Game (ICLR 2025, arXiv 2407.04503), found by threads A and
...
text-level counterpart of ATR's perturbation and basin-escape experiments. All [verified: search snippet].
...
character dominates. The located record transposes it to computational substrates three times. Backes, i am sitting in a machine
[verified: search snippet] https://www.martinbackes.com/i-am-sitting-in-a-machine/: an artificial voice through an MP3 encoder 3000 times, a
stated homage, the iterated operator a codec. Santos, The Degradation of Speech (2023) [verified: search snippet]
...
review) [verified: search snippet] https://arxiv.org/abs/2602.19033, found by threads A and D: proves a broad class of iterative feedback
...
Caveat: absence of evidence is bounded by search coverage (arXiv, ACL, NeurIPS, ICML, ICLR, LessWrong, Alignment Forum, surfaced via
snippets, horizon July 2026) and by the blocked direct fetches. A same-niche preprint using different vocabulary (for example "activation
recycling" or "representation feedback") could exist unindexed by the phrasings tried, though roughly a dozen distinct phrasings returned
none.

## Sources

URLs for the works cited above, deduplicated, grouped by thread; a work found by several threads is listed once, with the convergence noted.

Thread A (dynamical systems): https://github.com/locuslab/deq , https://wiki.math.ntnu.no/_media/ma2501/2014v/fixedpoint.pdf ,
...
https://arxiv.org/abs/2511.07384 , https://arxiv.org/pdf/2412.06769 (threads A, D), https://arxiv.org/abs/2103.03404 (threads A, B, D),
https://people.lids.mit.edu/yp/homepage/data/2023_transformers1.pdf , https://arxiv.org/abs/2407.09298 ,
https://www.lesswrong.com/posts/8mizBCm3dyc432nK8/residual-stream-norms-grow-exponentially-over-the-forward (threads A, B),
...
https://aclanthology.org/2025.acl-long.624/ and https://arxiv.org/abs/2502.15208 (threads A, C, D), https://arxiv.org/abs/2402.09221 ,
...
Thread B (GPT-2 mechanistic interpretability): https://transformer-circuits.pub/2021/framework/index.html (threads A, B),
...
Thread C (glitch tokens): https://www.lesswrong.com/posts/aPeJE8bSo6rAFoLqg/solidgoldmagikarp-plus-prompt-generation (threads A, B, C),
...
https://github.com/cohere-ai/magikarp (threads B, C), https://aclanthology.org/2024.emnlp-main.649/ , https://arxiv.org/abs/2404.09894 ,
...
https://arxiv.org/abs/2410.12318 , https://arxiv.org/abs/1702.01417 (threads B, C),
...
Thread D (text-level loops and lineage): https://arxiv.org/abs/1904.09751 , https://arxiv.org/abs/2206.02369 ,
https://www.translationparty.com , https://arxiv.org/abs/2407.04503 (threads A, D), https://arxiv.org/abs/2605.02236 ,
...
https://arxiv.org/abs/2602.19033 (threads A, D), https://arxiv.org/abs/2505.15778
