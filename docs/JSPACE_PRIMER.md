...
4. **The regime lesson travels both ways, and has since reversed once.** ATR's first null control seemed to show its basins belong to the language-driven regime; the corrected matched-ν re-run (run 17, 2026-07-31, matched on ν, the injected tensor's starting Frobenius norm) inverted that, with converged noise landing in the language arm's own basins (FINDINGS F4). The J-lens's averaged Jacobian, computed over a pretraining-like corpus, is by construction a language-regime object, which makes the question sharper rather than moot: what does a J-lens read from a converged *noise* attractor, now that noise attractors are largely THE language basins? (The earlier J-lens noise readings inherited the mis-calibrated arm: FINDINGS caveat 18.)
