...
That control did not survive its own audit. It had run at roughly a third of the intended injection scale and was counted before it settled ([FINDINGS.md caveat 18](FINDINGS.md#caveats)), and the corrected re-run (2026-07-31, run 17: each noise tensor matched to a real prompt's exact length and starting norm, counted only at convergence) reverses the verdict: the fully settled trials converge into seven basins, four of them the language arm's own five with `prolet` dominant in both arms, the two-beat trials decode to `Divine` itself among others, and over all 125 trials 97 (77.6%) land in the language arm's five. So at this injection scale the original framing was closer to right than its correction: **the basins are properties of the weights, revealed by noise just as readily as by language.** Whether that holds at other injection scales is the registered follow-up (the ν-sweep). The model-specific half survives untouched: GPT-2 Medium, trained on the same corpus, still collapses everything into a single empty token. The full evidence is in [FINDINGS.md](FINDINGS.md), finding F4.
