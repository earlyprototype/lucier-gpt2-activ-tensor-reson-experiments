...
This experiment implements iterative re-injection of the full residual stream tensor through the forward pass of a transformer language model (reference architecture: GPT-2 Small, 124M parameters, 12 layers, 12 heads, d_model=768). The residual stream at the final layer's `hook_resid_post` is extracted, L2-normalised, and re-injected at `blocks.0.hook_resid_pre` via a TransformerLens forward hook, overwriting the token embeddings. This is repeated for *N* iterations to map the attractor landscape of the iterated forward map **under the chosen initial-condition regime**. The observed landscape contains fixed points and at least one exact period-2 limit cycle (the `Divine` state; see [FINDINGS.md](FINDINGS.md), F9). The null-model control shows the landscape is regime-dependent, not a universal property of the weights (see [FINDINGS.md](FINDINGS.md), F4).

The same protocol runs cross-model via the shared engine (`atr_engine.py`): GPT-2 Medium (24 layers, d_model=1024), Pythia-160m (12 layers), Pythia-410m (24 layers): extraction always at the final layer's `resid_post`, injection at layer 0. A convergence-gated variant (`run_atr_gated`) classifies terminal basins at lock-in (`cos_sim_mean > 0.999` sustained over three consecutive checks) rather than at a fixed iteration horizon. The gate compares consecutive iterates (lag 1) by default; a lag-1 gate cannot pass a period-2 limit cycle by construction, so the engine also supports a `gate_lag` parameter (compare iterate t with t-k), under which the audited `Divine` trajectory is converged at `gate_lag = 2` (see [FINDINGS.md](FINDINGS.md), F15).

The process is a **nonlinear analogue of power iteration**: where classical power iteration converges to the dominant eigenvector of a linear operator, this procedure converges to attractors (fixed points and, in one observed case, an exact period-2 limit cycle) of the full transformer forward map *f*: ℝ^(seq×d) → ℝ^(seq×d), which includes LayerNorm, softmax attention (with dynamically recomputed QKV), GeLU MLP activations, and residual connections.
...
A direct empirical comparison between the last two rows above, the per-head resonant state actually observed under iterative re-injection versus the dominant singular vector predicted by static SVD of `W_OV`, was run in `experiments/gpt2_small/spectral_resonance.ipynb` (executed in the 2026-07-25 artifact regeneration: 5/144 heads above the 0.9 threshold, verdict NOT SUPPORTED as registered; artifacts in `experiments/_DATA/EXP_009/`). The 2026-07-31 operator ruling (issue #54) settled the disposition and superseded the comparison target: the isolated per-head loop applies `W_OV` transposed followed by a direction-preserving rescale, so its true target is the dominant eigenvector of `W_OV` transposed (equivalently a dominant left eigenvector of `W_OV`), not the top singular vector, and rescored against that target every settling head lands where the weights predict (run 16, `experiments/gpt2_small/output_eigen_rescore/report.md`, which retains the original scoring as a footnote). Note the caveat in the last row of the table applies to the full-stack loop, not the isolated per-head loop, which contains no nonlinearity beyond that direction-preserving rescale. See [FINDINGS.md](FINDINGS.md) (H4) for the canonical disposition.
