...
Inspired by Alvin Lucier's *I Am Sitting in a Room* (1969), this set of Jupyter notebooks applies an analogous process to GPT-2 Small, an early Large Language Model (LLM). As Lucier used the physical environment to explore the resonant frequencies of a room, this process uses the model's activation tensor as the environment. It proceeds by exciting the model's activation tensor through iterative forward-pass feedback (ie inputting a prompt, capturing the mathematical representation of the set of final output tokens, and then feeding that back into the model as the new prompt), repeating 500 times. As semantic content of the initial prompt dissolves, dominant attractor states emerge (sets of common output tokens that the model settles into), revealing the model's naked inner voice.
