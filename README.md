...
The two ends of that band do not look alike. Above it, each pass changes the tensor less and less, until the loop is merely copying. Below it the loop is not quiet at all: measured as the angle the state swings on each pass, it is turning harder as the band approaches, close to a right angle every time round, and the band is exactly where that swinging peaks. Whatever shuts the five notes off from below, it is not stillness. What does shut it off, we cannot yet say. An attempt to show that each end of the band is set by a different property of the gain did not survive its own audit and has been withdrawn; the description above stands, the explanation is still open ([FINDINGS.md caveat 19](docs/FINDINGS.md#caveats)).
