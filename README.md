...
![Cross-similarity matrix of basin tokens — all-warm, no negative correlations](docs/supervisor/token_similarity.png)
...
The review sessions ([Session 01](docs/supervisor/SESSION_01_SUPERVISORY_REVIEW.md), [Session 02](docs/supervisor/SESSION_02_RESULTS_DISCUSSION.md) — AI-assisted reviewer-persona sessions, not institutional supervision) cover the experimental design, results, and the outstanding statistical validation work in detail.
...
│       ├── stage1_results.pt                    ← Full activation trajectories (not committed — regenerated by the notebook)
...
│       └── dissolution_sentences.md             ← Full dissolution transcripts
│
...
├── archive/                                     ← Earlier notebook versions (incl. Phase 2 determinism check)
└── docs/                                        ← Documentation & analysis
    ...
    ├── ATR_COMPREHENSIVE_DASHBOARD.html         ← Interactive results dashboard
    ├── ATR_JOURNEY_VISUALIZATION.html           ← Interactive journey visualisation
    ├── prior_art/                               ← Prior-art search notes
    └── supervisor/                              ← AI-assisted review sessions
        ├── SESSION_01_SUPERVISORY_REVIEW.md     ← W_E neighbourhood analysis
        ├── SESSION_02_RESULTS_DISCUSSION.md     ← Bias theory, scaling plan
        └── 01_token_id_extraction.ipynb         ← Token ID extraction utilities
```

Phase 2 (the determinism check) has no committed top-level directory: `00_reproducibility_gate.ipynb` is a local artifact (see Validation above), and its archived predecessor lives in `archive/`. Notebook outputs such as `stage1_results.pt` are not committed — they are regenerated by the notebooks.
