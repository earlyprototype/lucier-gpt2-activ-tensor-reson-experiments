...
Inspired by Alvin Lucier's *I Am Sitting in a Room* (1969), this set of Jupyter notebooks applies an analogous process to the internal representational geometry of GPT-2 Small, an early Large Language Model (LLM). Lucier's process piece probed the resonant frequencies of a physical space through looped excitation: sound was played into a room, the reverberant output captured, and that recording fed back as the next input. Here, the analogous bounded environment is the model's _activation tensor_.
