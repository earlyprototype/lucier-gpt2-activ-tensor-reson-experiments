...
That follow-up ran (2026-08-02, run 18: the ν-sweep, 1,025 trials over Stages A and B). Turn the playback level and the answer arrives in strata: the five notes exist only in a band, from about 64 to at least 256 times the loudness the room naturally receives, and the level our recordings always used sits inside it. Played quieter, the room does not go silent; it steps through entirely different words, one per stratum (`arbit`, then a bar of punctuation, then a splintering into fourteen), and the quietest strata point into the model's least-trained corner of the vocabulary, the same anomalous-token cluster the flip axis found. Played louder still, the sound thins into `the` and full stops as each pass changes the tensor less and less. The room's tone depends on the gain ([FINDINGS.md caveat 19](docs/FINDINGS.md#caveats)); where the band's edges fall exactly, and what sets them, is the running Stage C question (issue #116).
