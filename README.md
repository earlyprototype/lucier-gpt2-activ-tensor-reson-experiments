...
- A null control relocated the basins: **pure noise converges into 18 non-semantic attractors** with no overlap with the real five. The basins belong to the *language-driven regime* of the model, not to the weights in general. (The noise arm is now known to have run at ~1/√T of the intended injection norm and to have been counted before convergence, so the basin-count comparison is confounded and the control is queued for a corrected re-run: [FINDINGS.md caveat 18](docs/FINDINGS.md#caveats), issue #97.)
- The 34 prompts that never pass the lag-1 convergence gate are exactly the 34 `Divine` prompts, resolved as an exact **period-2 limit cycle**: the tensor alternates between two states forever while the decoded word never changes; a lag-2 gate classifies the audited trajectory as converged ([FINDINGS.md](docs/FINDINGS.md), F9, F15).
