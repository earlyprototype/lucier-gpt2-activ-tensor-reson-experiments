"""
Comprehensive Prompt Library for Attractor Dominance Testing (EXP_009d1)
=======================================================================
125 prompts grouped by syntactic/semantic register (the independent
variable). Predictions are stated separately and do NOT influence prompt
grouping.

Runtime estimate: ~60 minutes on GPU at MAX_ITERATIONS=100.

------------------------------------------------------------------------
PROVENANCE (issue #24 — supersedes the "restore the original file" framing
of issue #9)
------------------------------------------------------------------------
This file is a provenance-flagged reconstruction. The task called for
recovering every prompt that could be recovered from committed artifacts
before authoring any replacements, and for tagging each entry as either
`original` (recovered verbatim from a committed record) or
`reconstructed-new` (authored in-register to fill a gap).

Result of the recovery pass: ALL 125 prompts were recovered verbatim.
Two independent committed records agree exactly, so there are no gaps and
no `reconstructed-new` entries.

Recovery sources (all cross-checked, byte-for-byte agreement):
  1. Deleted git blob `B_AttractorDominance/prompt_library.py`
     (object 2931d42; removed in commits 9882d57 "remove ... duplicate
     root prompt_library.py" and d91049e "cross-model ATR extension —
     restructure"). This is the original file itself, recovered from
     history — the full 125 prompts, seven register dicts, PREDICTIONS
     and CATEGORY_MAP.
  2. `experiments/gpt2_small/output/dissolution_sentences.md` — the
     committed dissolution record quotes all 125 prompts verbatim with
     their IDs and categories ("Full Sentence Dissolution: All 125
     Prompts"). Independently reproduces every prompt in source 1.
  3. Cross-check: `ATR_research` repo
     `_STAGE2_JSPACE/experiments/exp_010c_windows/output/prompt_subset.json`
     — 25 recovered prompts (one per register mix) derived by
     `derive_prompts.py`; all 25 match sources 1 and 2 exactly.

Per-register split (original / reconstructed-new):
  Complex    25 / 0
  Narrative  20 / 0
  Simple     20 / 0
  Chemical   10 / 0
...
Because every entry is `original`, a 125-prompt sweep run against this
file IS a reproduction of the published Stage 1 basin table, not a new
experiment. The `PROVENANCE` dict below records the flag and source for
each entry so that downstream reporting (chordness at scale #10, the bell
census #14) can inherit the flags. Should any entry ever be replaced with
a `reconstructed-new` prompt, flip its flag here and the reproduction
claim for that sweep no longer holds.
"""
...
# ============================================================
# PROVENANCE (per issue #24)
# ============================================================
# Maps each prompt ID to (flag, source):
#   flag   — "original" (recovered verbatim from a committed record)
#            or "reconstructed-new" (authored in-register to fill a gap)
#   source — short tag identifying the committed record it was recovered
#            from, or "authored" for reconstructed-new entries.
#
# All 125 entries are "original", recovered verbatim from the deleted git
# blob B_AttractorDominance/prompt_library.py (object 2931d42) and
# independently confirmed by experiments/gpt2_small/output/
# dissolution_sentences.md. There are no reconstructed-new entries.

_ORIGINAL_SOURCE = "git-blob:2931d42 + dissolution_sentences.md"

PROVENANCE = {k: ("original", _ORIGINAL_SOURCE) for k in PROMPT_LIBRARY}

assert set(PROVENANCE) == set(PROMPT_LIBRARY), "PROVENANCE must cover every prompt"

# Convenience split counts (original, reconstructed-new).
PROVENANCE_COUNTS = {
    "original": sum(1 for f, _ in PROVENANCE.values() if f == "original"),
    "reconstructed-new": sum(1 for f, _ in PROVENANCE.values() if f == "reconstructed-new"),
}
assert PROVENANCE_COUNTS["original"] == 125, PROVENANCE_COUNTS


if __name__ == "__main__":
    print(f"Total prompts: {len(PROMPT_LIBRARY)}")
    print(f"Provenance split: {PROVENANCE_COUNTS}")
