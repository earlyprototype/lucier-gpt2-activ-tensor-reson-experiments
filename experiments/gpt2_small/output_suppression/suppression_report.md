...
- **The mechanism is confirmed and sharpened.** The flip axis inversion that sustains the cycle is the static OV geometry of one head: among the directions tested (d_sym and five random controls), d_sym is the only one that L11.H8's OV both inverts (cos -0.9619) and amplifies (gain 63.7), and along that tested flip axis L11.H8 is rank 1 of all 144 heads; the empirical operating-point response matches the linear computation exactly once the ln1 scale is accounted for, and the closed loop needs exactly this head: ablate it and the cycle is replaced by a fixed point with a generic readout within about 10 iterations; ablate a neighbour and the cycle persists.
